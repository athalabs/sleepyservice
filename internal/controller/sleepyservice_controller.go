...
func (r *SleepyServiceReconciler) buildProxyEnv(hs *sleepyv1alpha1.SleepyService) []corev1.EnvVar {
	...
	if appComponent != nil {
		...
		// Legacy BACKEND_URL (for backwards compatibility, uses first port)
		firstPort := r.getBackendServicePort(hs)
		backendURL := fmt.Sprintf("http://%s:%d", backendHost, firstPort)

		env = append(env,
			corev1.EnvVar{Name: "DEPLOYMENT_NAME", Value: appComponent.Ref.Name},
			corev1.EnvVar{Name: "BACKEND_URL", Value: backendURL},                        // Legacy
			corev1.EnvVar{Name: "BACKEND_HOST", Value: backendHost},                      // New
			corev1.EnvVar{Name: "BACKEND_PORTS", Value: strings.Join(backendPorts, ",")}, // New
		)
